...
async function main() {
  ...
  console.log("=== Últimos 10 Archivos Subidos para Empresa 117 ===");
  for (const a of archivos) {
    ...
    console.log(`  -> Emisor: ${emisor?.nombre} (CIF: ${emisor?.identificador_fiscal})`);
    console.log(`  -> Cliente: ${cliente?.nombre} (CIF: ${cliente?.identificador_fiscal})`);
    ...
    if (audit && audit.detalles) {
      try {
        const details = JSON.parse(audit.detalles as string);
        if (details.rawJson) {
