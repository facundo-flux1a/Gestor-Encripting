...
const calculateAnnualSummary = (data: Document[], selectedPeriodos?: Set<string>) => {
    ...
    data.forEach((doc) => {
        ...
        if (!hasIvaDetails) {
            ...
            let refinedIva = Math.max(0, Math.abs(docTotalVal) - refinedBase + docDescuentoGlobal - recCab + retCab);
...
export function TrimestreExcelView({ documents, isLoading, año, selectedTrimestre, selectedPeriodos }: TrimestreExcelViewProps) {
    ...
    const columnDefs = useMemo<any[]>(() => [
        {
            ...
            tooltipValueGetter: (p: any) => {
                ...
                if (p.data?.mismatches) {
                    let tooltipLines = ['⚠️ ALERTA DE DESCUADRE:'];
                    tooltipLines.push('La cuota de IVA extraída no coincide matemáticamente con la base declarada para las siguientes facturas:\n');
                    
                    [1, 2, 3, 4].forEach(q => {
                        if (p.data.mismatches[q] && p.data.mismatches[q].length > 0) {
                            tooltipLines.push(`Trimestre ${q}:`);
                            p.data.mismatches[q].forEach((m: string) => tooltipLines.push(`• ${m}`));
                            tooltipLines.push('');
                        }
                    });
        ...
        },
        {
            ...
            tooltipValueGetter: (p: any) => {
                if (p.data?.mismatches && p.data.mismatches[1] && p.data.mismatches[1].length > 0) {
                    return `⚠️ DESCUADRE MATEMÁTICO (1º TRIMESTRE)\n\nSe detectó una discrepancia matemática en la línea de impuestos. Los valores extraídos de Base Imponible, Cuota de IVA o Porcentaje no son coherentes entre sí (Base × % ≠ Cuota).\n\nDocumentos afectados:\n` + p.data.mismatches[1].map((m: string) => `• ${m}`).join('\n') + `\n\n💡 Acción: Verifique los valores de la Base Imponible, Cuota y Porcentaje en el desglose de impuestos del documento.`;
                }
                return undefined;
            },
            cellStyle: (p: any) => ({
                textAlign: 'right',
                backgroundColor: p.data?.mismatches?.[1]?.length > 0 ? 'rgba(245, 158, 11, 0.2)' : (selectedTrimestre === 1 ? 'rgba(59, 130, 246, 0.15)' : ''),
                color: p.data?.mismatches?.[1]?.length > 0 ? '#f59e0b' : '',
                fontWeight: p.data?.mismatches?.[1]?.length > 0 ? 'bold' : '',
        ...
        },
        {
            ...
            valueFormatter: currencyFormatter,
            tooltipValueGetter: (p: any) => {
                if (p.data?.mismatches && p.data.mismatches[2] && p.data.mismatches[2].length > 0) {
                    return `⚠️ DESCUADRE MATEMÁTICO (2º TRIMESTRE)\n\nSe detectó una discrepancia matemática en la línea de impuestos. Los valores extraídos de Base Imponible, Cuota de IVA o Porcentaje no son coherentes entre sí (Base × % ≠ Cuota).\n\nDocumentos afectados:\n` + p.data.mismatches[2].map((m: string) => `• ${m}`).join('\n') + `\n\n💡 Acción: Verifique los valores de la Base Imponible, Cuota y Porcentaje en el desglose de impuestos del documento.`;
                }
                return undefined;
            },
            cellStyle: (p: any) => ({
                textAlign: 'right',
                backgroundColor: p.data?.mismatches?.[2]?.length > 0 ? 'rgba(245, 158, 11, 0.2)' : (selectedTrimestre === 2 ? 'rgba(59, 130, 246, 0.15)' : ''),
                color: p.data?.mismatches?.[2]?.length > 0 ? '#f59e0b' : '',
                fontWeight: p.data?.mismatches?.[2]?.length > 0 ? 'bold' : '',
        ...
        },
        {
            ...
            valueFormatter: currencyFormatter,
            tooltipValueGetter: (p: any) => {
                if (p.data?.mismatches && p.data.mismatches[3] && p.data.mismatches[3].length > 0) {
                    return `⚠️ DESCUADRE MATEMÁTICO (3º TRIMESTRE)\n\nSe detectó una discrepancia matemática en la línea de impuestos. Los valores extraídos de Base Imponible, Cuota de IVA o Porcentaje no son coherentes entre sí (Base × % ≠ Cuota).\n\nDocumentos afectados:\n` + p.data.mismatches[3].map((m: string) => `• ${m}`).join('\n') + `\n\n💡 Acción: Verifique los valores de la Base Imponible, Cuota y Porcentaje en el desglose de impuestos del documento.`;
                }
                return undefined;
            },
            cellStyle: (p: any) => ({
                textAlign: 'right',
                backgroundColor: p.data?.mismatches?.[3]?.length > 0 ? 'rgba(245, 158, 11, 0.2)' : (selectedTrimestre === 3 ? 'rgba(59, 130, 246, 0.15)' : ''),
                color: p.data?.mismatches?.[3]?.length > 0 ? '#f59e0b' : '',
                fontWeight: p.data?.mismatches?.[3]?.length > 0 ? 'bold' : '',
        ...
        },
        {
            ...
            valueFormatter: currencyFormatter,
            tooltipValueGetter: (p: any) => {
                if (p.data?.mismatches && p.data.mismatches[4] && p.data.mismatches[4].length > 0) {
                    return `⚠️ DESCUADRE MATEMÁTICO (4º TRIMESTRE)\n\nSe detectó una discrepancia matemática en la línea de impuestos. Los valores extraídos de Base Imponible, Cuota de IVA o Porcentaje no son coherentes entre sí (Base × % ≠ Cuota).\n\nDocumentos afectados:\n` + p.data.mismatches[4].map((m: string) => `• ${m}`).join('\n') + `\n\n💡 Acción: Verifique los valores de la Base Imponible, Cuota y Porcentaje en el desglose de impuestos del documento.`;
                }
                return undefined;
            },
            cellStyle: (p: any) => ({
                textAlign: 'right',
                backgroundColor: p.data?.mismatches?.[4]?.length > 0 ? 'rgba(245, 158, 11, 0.2)' : (selectedTrimestre === 4 ? 'rgba(59, 130, 246, 0.15)' : ''),
                color: p.data?.mismatches?.[4]?.length > 0 ? '#f59e0b' : '',
                fontWeight: p.data?.mismatches?.[4]?.length > 0 ? 'bold' : '',
        ...
        const buildRows = (summary: any, accentColor: string) => {
            ...
            currentRates.forEach(rate => {
                ...
                if (summary[keyIva] && (summary[keyIva].total !== 0 || quarters.some(q => summary[keyBase]?.[q] !== 0))) {
                    const mismatches = summary.iva_docs[rate];
                    const hasMismatch = mismatches && (mismatches[1]?.length > 0 || mismatches[2]?.length > 0 || mismatches[3]?.length > 0 || mismatches[4]?.length > 0);
                    
                    const row: any = {
                        concepto: hasMismatch ? `⚠️ IVA ${rate}%` : `IVA ${rate}%`,
                        ...
                        mismatches: hasMismatch ? mismatches : null
                    };
    ...
    return (
        <Card className="mb-8 w-full overflow-hidden border-border bg-card shadow-sm transition-shadow duration-300 hover:shadow-md">
            <CardHeader
                ...
                {isExpanded && (
                    ...
                    >
                        <CardContent className="p-0">
                            ...
                            {/* AG Grids */}
                            <div className="flex flex-col gap-8 bg-muted/20 p-6">
