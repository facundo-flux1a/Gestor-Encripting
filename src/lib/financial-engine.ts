...
export interface FinancialSummary {
    ...
    descuentos: Record<number | string, number>;
...
const createEmptySummary = (): FinancialSummary => {
    const summary: FinancialSummary = {
        ...
        descuentos: { 1: 0, 2: 0, 3: 0, 4: 0, total: 0 },
...
export function calculateFinancials(documents: Document[], companyCIF: string | null): EngineResult {
    ...
    documents.forEach(doc => {
        ...
        if (ivaRealVal > 0 && recargoCab > 0 && Math.abs(ivaRealVal - (valReal - baseRealVal + docDescuentoGlobal)) < 0.01) {
        ...
        if (ivaRealVal === 0) {
            ivaRealVal = Math.max(0, valReal - baseRealVal + docDescuentoGlobal - recargoCab + retencionCab);
        }
        ...
        totalNeto.descuentos.total += docDescuentoGlobal * netoSign;
        ...
        if (ivaDetails.length > 0) {
            ...
            const diff = valReal - baseRealVal + docDescuentoGlobal;
        ...
        const ivaFallback = valReal - baseRealVal + docDescuentoGlobal;
